...
class ClientConnection():
    ...
    """
        This class represents a active asynchronus connection between a client and a server
        using tor as a proxy.

        The connection run in the same event loop from the controller in a asynchronus task
        ...
        server_task : asyncio.Task
            Stores the task responsible to run the asynchronus connection
        
        Methods
        --------

        run()
            Initialize the queues inside the asynchronus event loop context an start the 
            connection process
        close_connection()
            Shuts down the connection asynchronus task and define the state as not connected
        start_connection()
